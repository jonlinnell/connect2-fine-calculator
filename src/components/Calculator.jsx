...
const Calculator = () => {
  ...
  const processOverdueRecords = useCallback(records => {
    setOverdueLoans(records.map(record => {
      const dailyFine = fines.find(fine => (fine.resource === record['Parent resource'] || fine.resource === record.Resource)).amount || 0;
  ...
  const handleFile = useCallback(file => {
    ...
    reader.onload = () => {
      const fileContents = reader.result
        .replace('Barcode', 'Resource Barcode');
  ...
  return (
    <div>
      ...
      <div style={{ maxHeight: '1400px', width: '80vw', overflow: 'scroll' }}>
          ...
          <tbody>
            {overdueLoans.map(loan => (
              <tr key={`${loan['Ref no']}-${loan['Resource Barcode']}`}>
                <td><a href={`https://lborolondon.getconnect2.com/bookings/view.aspx?id=${loan['Ref no'].slice(-6)}`}>{loan['Ref no']}</a></td>
                <td>{loan['First name']} {loan.Surname}</td>
                <td>{loan.Barcode}</td>
                <td>{loan.Email}</td>
                <td>
                  {
                    loan.Barcode
                      ? (<span>({loan['Resource Barcode']})&nbsp;</span>)
                      : null
                  }
                {loan.Resource}</td>
